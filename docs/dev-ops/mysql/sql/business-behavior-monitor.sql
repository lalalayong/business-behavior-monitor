CREATE
database if NOT EXISTS `business-behavior-monitor` default character set utf8mb4 ;
use
...
(
    `id`                   int(10) unsigned NOT NULL AUTO_INCREMENT,
    `monitor_id`           varchar(8) NOT NULL COMMENT '监控ID',
    `from_monitor_node_id` varchar(8) NOT NULL COMMENT 'from 节点ID',
    `to_monitor_node_id`   varchar(8) NOT NULL COMMENT 'to 节点ID',
    `create_time`          datetime   NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
    `update_time`          datetime   NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间',
    PRIMARY KEY (`id`)
